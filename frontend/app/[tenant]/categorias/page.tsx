      ...
      <div className="card">
        {loading ? (
          ...
          <ResponsiveTable
            ...
            columns={[
              { key: 'nombre', header: 'Nombre', render: (c) => <span style={{ fontWeight: 600, color: 'var(--text)', fontSize: 13 }}>{c.name}</span> },
              { key: 'slug', header: 'Slug', render: (c) => <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text3)' }}>{c.slug}</span> },
              { key: 'productos', header: 'Productos', render: (c) => <span style={{ fontFamily: 'var(--mono)', fontSize: 12 }}>{c._count?.products ?? '—'}</span> },
              { key: 'estado', header: 'Estado', render: (c) => <span className={`badge ${c.isActive ? 'badge-green' : 'badge-gray'}`}>{c.isActive ? 'Activa' : 'Inactiva'}</span> },
              {
            ...
            ] as ResponsiveTableColumn<ProductCategory>[]}
            renderMobileCard={(c) => (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div className="mobile-card-head">
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{c.name}</span>
                  <span className={`badge ${c.isActive ? 'badge-green' : 'badge-gray'}`}>{c.isActive ? 'Activa' : 'Inactiva'}</span>
                </div>
                <div className="mobile-card-row">
                  <span style={{ fontFamily: 'var(--mono)', color: 'var(--text3)' }}>{c.slug}</span>
                  <span style={{ fontFamily: 'var(--mono)' }}>{c._count?.products ?? '—'} productos</span>
                </div>

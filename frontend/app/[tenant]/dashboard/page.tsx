        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div className="hero-header">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 24, fontWeight: 800, marginTop: 4, letterSpacing: -0.5 }}>
                  {fmtMoney(d?.today?.revenue ?? 0)}
                </div>
                <div style={{ fontSize: 11.5, opacity: 0.85, marginTop: 3, fontFamily: 'var(--mono)' }}>
                  {d?.today?.salesCount ?? 0} tickets hoy
                </div>
              </div>
              <ProgressRing
                value={d?.today?.grossMarginPercent ?? 0}
                color="#fff"
                size={58}
                label={<span style={{ color: '#fff', fontSize: 12 }}>{Math.round(d?.today?.grossMarginPercent ?? 0)}%</span>}
              />
            </div>
          </div>
          ...
          <div className="card">
            <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <a href={`/${tenant}/ventas`} className="btn btn-ghost btn-sm" style={{ gap: 5 }}>Ver todas <ArrowUpRight size={13} /></a>
            </div>
            {todaySales.length === 0 ? (

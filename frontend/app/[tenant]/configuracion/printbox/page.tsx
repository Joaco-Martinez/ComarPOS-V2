...
import { Printer, Plus, X, Edit2, Trash2, RefreshCcw, Clock, Wifi } from 'lucide-react';
...
export default function PrintboxPage() {
  ...
  const openCreate = () => { setNewName(''); setCreateModal(true); };

  const create = async () => {
    if (!newName.trim()) return;
    setCreating(true);
    try {
      await api.post('/printbox/devices', { name: newName.trim() });
      setCreateModal(false);
      showToast('PrintBox creado — copiá el código antes de que expire');
      load();
    } catch (err: any) {
  ...
  return (
    <AppLayout
      title="PrintBox"
      subtitle="Impresoras térmicas conectadas por ESP32 (sin cable USB a la caja)"
      actions={
        <button onClick={openCreate} className="btn btn-primary btn-sm" style={{ gap: 6 }}>
          <Plus size={13} /> Nuevo PrintBox
        </button>
      }
    >
      {toast && (
      ...
      {loading ? (
        ...
        <div className="card empty-state" style={{ padding: 60 }}>
          <Printer size={40} />
          <p>Sin PrintBox registrados</p>
          <p style={{ fontSize: 12, color: 'var(--text3)', marginTop: 4 }}>
            Creá uno y vas a recibir un código para emparejar el dispositivo físico.
          </p>
        </div>
      ) : (
          ...
          {devices.map((d) => {
            ...
            return (
              <div key={d.id} className="card" style={{ padding: '16px 18px' }}>
                ...
                {d.status === 'PENDING_PAIRING' && !codeExpired && (
                  <div style={{
        ...
        <div className="modal-overlay" onClick={() => setCreateModal(false)}>
          <div className="modal" style={{ maxWidth: 420 }} onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <span style={{ fontWeight: 800 }}>Nuevo PrintBox</span>

...
function SimpleTable({ cols, rows }: { cols: { key: string; label: string; mono?: boolean; money?: boolean }[]; rows: any[] }) {
  const data = rows.map((r, i) => ({ ...r, __rtKey: i }));
  const cellValue = (c: { key: string; money?: boolean }, r: any) => (c.money ? fmtMoney(num(r[c.key])) : (r[c.key] ?? '—'));
  return (
    <ResponsiveTable
      data={data}
      keyFor={(r) => r.__rtKey}
      ...
      columns={cols.map((c) => ({
        ...
        header: c.label,
        render: (r) => (
          <span style={{ fontFamily: c.mono ? 'var(--mono)' : undefined, color: c.mono ? 'var(--text2)' : 'var(--text)' }}>
            {cellValue(c, r)}
          </span>
        ),
      ...
      renderMobileCard={(r) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div className="mobile-card-head">
            <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{cellValue(cols[0], r)}</span>
          </div>
          {cols.slice(1).map((c) => (
            <div className="mobile-card-row" key={c.key}>
              <span>{c.label}</span>
              <span style={{ fontFamily: c.mono ? 'var(--mono)' : undefined }}>{cellValue(c, r)}</span>
            </div>
          ))}
        </div>
      )}

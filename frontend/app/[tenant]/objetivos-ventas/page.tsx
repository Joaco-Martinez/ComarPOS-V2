      ...
      <div className="card">
        {loading ? (
          ...
          <ResponsiveTable
            ...
            emptyMessage="Sin objetivos registrados"
            columns={[
              { key: 'nombre', header: 'Nombre', render: (p) => <span style={{ fontWeight: 600, fontSize: 13 }}>{p.goal.title}</span> },
              { key: 'metrica', header: 'Métrica', render: (p) => <span style={{ fontSize: 12, color: 'var(--text3)' }}>{metricLabel[p.goal.metric]}</span> },
              {
                key: 'periodo', header: 'Período',
                render: (p) => <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text3)' }}>{fmtDate(p.goal.periodStart)} → {fmtDate(p.goal.periodEnd)}</span>,
              },
              {
                key: 'meta', header: 'Meta', style: { textAlign: 'right' },
                render: (p) => <span style={{ fontFamily: 'var(--mono)', fontWeight: 700 }}>{fmtMoney(p.goal.targetAmount)}</span>,
              },
              {
                key: 'progreso', header: 'Progreso', style: { textAlign: 'right' },
                render: (p) => {
                  const pct = Math.min(num(p.progressPercent), 100);
                  return <span style={{ fontFamily: 'var(--mono)', fontSize: 12, color: pct >= 100 ? 'var(--success)' : 'var(--accent)' }}>{pct.toFixed(1)}%</span>;
                },
              },
              {
                key: 'estado', header: 'Estado', render: (p) => {
                  const pct = Math.min(num(p.progressPercent), 100);
                  const status = goalStatus(p.goal, pct);
                  return <span className={`badge ${statusBadge[status]}`}>{status.replace('_', ' ')}</span>;
                },
              },
              {
            ...
            ] as ResponsiveTableColumn<Progress>[]}
            renderMobileCard={(p) => {
              const g = p.goal;
              const pct = Math.min(num(p.progressPercent), 100);
              ...
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <div className="mobile-card-row">
                    ...
                    <span style={{ fontFamily: 'var(--mono)', color: 'var(--text3)' }}>{fmtDate(g.periodStart)} → {fmtDate(g.periodEnd)}</span>
                  </div>
                  <div className="mobile-card-row">
                    <span>Meta: <span style={{ fontFamily: 'var(--mono)', fontWeight: 700 }}>{fmtMoney(g.targetAmount)}</span></span>
                    <span style={{ fontFamily: 'var(--mono)', fontWeight: 700, color: pct >= 100 ? 'var(--success)' : 'var(--accent)' }}>{pct.toFixed(1)}%</span>
                  </div>

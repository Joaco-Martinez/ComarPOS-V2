      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <ResponsiveTable
            ...
            emptyMessage="Sin tasas registradas"
            columns={[
              { key: 'fecha', header: 'Fecha', render: (r) => <span style={{ fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text3)' }}>{fmtDate(r.createdAt)}</span> },
              { key: 'moneda', header: 'Moneda', render: (r) => <span className="badge badge-cyan">{r.currency}</span> },
              {
                key: 'tasa', header: 'Tasa (→ ARS)', style: { textAlign: 'right' },
                render: (r) => <span style={{ fontFamily: 'var(--mono)', fontWeight: 700, fontSize: 13 }}>{num(r.rate).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>,
              },
            ...
            renderMobileCard={(r) => (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <div className="mobile-card-head">
                  <span style={{ fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text3)' }}>{fmtDate(r.createdAt)}</span>
                  <span className="badge badge-cyan">{r.currency}</span>
                </div>
                <div className="mobile-card-row">
                  <span>Tasa (→ ARS)</span>
                  <span style={{ fontFamily: 'var(--mono)', fontWeight: 700 }}>{num(r.rate).toLocaleString('es-AR', { minimumFractionDigits: 2 })}</span>
                </div>
                {r.source && (
                  <div className="mobile-card-row">
                    <span>Notas</span>

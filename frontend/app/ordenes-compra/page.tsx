            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['#', 'Proveedor', 'Fecha esperada', 'Estado', 'Items', 'Total estimado', 'Acciones'].map((h) => (
              ...
              <tbody>
                {orders.length === 0 && (
                  <tr><td colSpan={7} style={{ padding: 40, textAlign: 'center', color: 'var(--text3)' }}>No hay órdenes</td></tr>
                )}
                {orders.map((ord) => (
                  <tr key={ord.id} style={{ borderBottom: '1px solid var(--border)' }}
                    ...
                    <td style={{ padding: '10px 14px', color: 'var(--text2)' }}>{ord.expectedDate ? fmtDate(ord.expectedDate) : '—'}</td>
                    <td style={{ padding: '10px 14px' }}>
                      <span className={`badge ${statusBadge[ord.status as Status] ?? 'badge-amber'}`}>{statusLabel[ord.status as Status] ?? ord.status}</span>
                    </td>
                    <td style={{ padding: '10px 14px', color: 'var(--text2)' }}>{(ord.items ?? []).length}</td>
                    <td style={{ padding: '10px 14px', fontFamily: 'var(--mono)', color: 'var(--accent)' }}>{fmtMoney(estimatedTotal(ord))}</td>
                    <td style={{ padding: '10px 14px' }}>

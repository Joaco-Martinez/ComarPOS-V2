        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18, maxWidth: 860 }}>
          ...
          >
            ...
            <div className="table-wrap">
              <table>
                <thead><tr><th>PV</th><th>Descripción</th><th>Tipos</th><th>Estado</th><th></th></tr></thead>
                <tbody>
                  {points.length === 0 ? (
                    <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--text3)', padding: 24 }}>Sin puntos de venta</td></tr>
                  ) : points.map(pv => (
                    <tr key={pv.id}>
                      <td style={{ fontFamily: 'var(--mono)', fontWeight: 700 }}>{String(pv.number).padStart(4, '0')}</td>
                      <td>{pv.description ?? '—'}</td>
                      <td style={{ fontFamily: 'var(--mono)', fontSize: 12 }}>{pv.enabledCbteTypes?.join(', ') ?? '—'}</td>
          ...
          >
            ...
            <div className="table-wrap">
              <table>
                <thead><tr><th>PV</th><th>CAI</th><th>Vence</th><th>Rango</th><th>Próximo</th><th>Estado</th><th></th></tr></thead>
                <tbody>
                  {remitoCais.length === 0 ? (
                    <tr><td colSpan={7} style={{ textAlign: 'center', color: 'var(--text3)', padding: 24 }}>Sin CAI de remitos cargado</td></tr>
                  ) : remitoCais.map(item => {
                    const expired = new Date(item.expiresAt).getTime() < NOW_TS;
                    return (
                      <tr key={item.id}>
                        <td style={{ fontFamily: 'var(--mono)', fontWeight: 700 }}>{String(item.pointOfSale).padStart(4, '0')}</td>
                        <td style={{ fontFamily: 'var(--mono)', fontSize: 12 }}>{item.cai}</td>
                        <td style={{ fontFamily: 'var(--mono)', fontSize: 12 }}>{fmtDate(item.expiresAt)}</td>
                        <td style={{ fontFamily: 'var(--mono)', fontSize: 12 }}>{item.rangeFrom ?? '—'} a {item.rangeTo ?? '—'}</td>
                        <td style={{ fontFamily: 'var(--mono)', fontSize: 12 }}>{item.nextNumber ?? '—'}</td>
                        <td>

            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Número', 'Fecha', 'Cliente', 'Venta', 'Estado', 'Acciones'].map((h) => (
                    <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 11, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {remitos.map((r) => (
                  ...
                  >
                    <td style={{ padding: '10px 14px', fontFamily: 'var(--mono)', fontWeight: 700, color: 'var(--accent)' }}>
                      #{r.numero ?? r.id?.slice(0, 8)}
                    </td>
                    <td style={{ padding: '10px 14px', color: 'var(--text2)' }}>{fmtDate(r.createdAt ?? r.date)}</td>
                    <td style={{ padding: '10px 14px' }}>
                      {r.sale?.client
                        ? `${r.sale.client.nombre ?? ''} ${r.sale.client.apellido ?? ''}`.trim() || '—'
                        : r.clientName ?? '—'}
                    </td>
                    <td style={{ padding: '10px 14px', fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--text3)' }}>
                      {r.saleId ? r.saleId.slice(0, 8) : '—'}
                    </td>
                    <td style={{ padding: '10px 14px' }}>

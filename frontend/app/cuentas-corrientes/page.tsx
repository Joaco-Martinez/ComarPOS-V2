            ...
            <>
              ...
              <div className="card" style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                <div style={{ overflowY: 'auto', flex: 1 }}>
                  {movements.length === 0 ? (
                    <div className="empty-state"><CreditCard size={28} /><p>Sin movimientos</p></div>
                  ) : (
                    <div className="table-wrap">
                      <table>
                        <thead>
                          <tr><th>Fecha</th><th>Tipo</th><th>Método</th><th>Descripción</th><th style={{ textAlign: 'right' }}>Monto</th><th style={{ textAlign: 'right' }}>Saldo</th></tr>
                        </thead>
                        <tbody>
                          {movements.map((m) => (
                            <tr key={m.id}>
                              <td style={{ fontFamily: 'var(--mono)', fontSize: 11 }}>{fmtDate(m.date)}</td>
                              <td><span className={`badge ${typeBadge(m.type)}`}>{typeLabel[m.type] ?? m.type}</span></td>
                              <td style={{ fontSize: 11, fontFamily: 'var(--mono)' }}>{m.paymentMethod ?? '—'}</td>
                              <td style={{ fontSize: 12, color: 'var(--text3)' }}>{m.description ?? (m.sale ? `Venta #${m.saleId?.slice(-6)}` : '—')}</td>
                              <td style={{ textAlign: 'right', fontFamily: 'var(--mono)', fontWeight: 700, color: m.type === 'PAYMENT' ? 'var(--success)' : 'var(--danger)', fontSize: 13 }}>
                                {m.type === 'PAYMENT' ? '+' : '−'}{fmtMoney(Math.abs(num(m.amount)))}
                              </td>
                              <td style={{ textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 12, color: num(m.newBalance) < 0 ? 'var(--danger)' : 'var(--text2)' }}>

...
async function main() {
  ...
  const icon192 = await squareOnWhite(192, 0.02);
  fs.writeFileSync(path.join(ICONS_DIR, 'icon-192.png'), icon192);

  const icon512 = await squareOnWhite(512, 0.02);
  ...
  const maskable512 = await squareOnWhite(512, 0.12);
  fs.writeFileSync(path.join(ICONS_DIR, 'maskable-512.png'), maskable512);

  const apple180 = await squareOnWhite(180, 0.03);
